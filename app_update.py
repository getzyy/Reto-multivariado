...
def load_and_clean_data():
    try:
        ...
        df_anderol = pd.read_csv("Anderol.csv")
        df_solgelsal = pd.read_csv("Colgelsa - Hoja 1.csv")
        df_starplex = pd.read_csv("Starplex.csv")
        df_chevron = pd.read_csv("Chevron.csv")
    ...
    df_chevron = limpiar_columnas(df_chevron)
    ...
    df_chevron = df_chevron[columnas_modelo].copy()

    df_base["Empresa"] = "Interlub"
    df_anderol["Empresa"] = "Anderol"
    df_solgelsal["Empresa"] = "Solgelsal"
    df_starplex["Empresa"] = "Starplex"
    df_chevron["Empresa"] = "Chevron"

    df_total = pd.concat([df_base, df_anderol, df_solgelsal, df_starplex, df_chevron], ignore_index=True)
